...
Develop a standalone Go web application that serves as a local recipe dashboard. Users can create recipes (including uploading a photo and listing ingredients), which are saved to SQLite. Users can trigger adding a recipe's ingredients to Todoist via two methods:
...
├── data/recipes.db       # Persistent SQLite storage for recipes
...
POST	/api/recipes	Receives multipart/form-data, saves the image to /uploads/, and inserts recipe data into SQLite.
POST	/api/push/{id}	UI-triggered endpoint. Reads recipe {id} from SQLite, pushes ingredients to Todoist.
...
A. Data Management (SQLite)

The app must safely read/write recipes in SQLite. A `recipes` table should include `id`, `name`, `image_path`, and ingredients stored as JSON text.

B. Image Upload Handling

    The "Add Recipe" HTML form must use enctype="multipart/form-data".

    The Go backend must parse the incoming file, generate a unique filename (e.g., using time.Now().UnixNano() to prevent overwrites), save it to the /uploads/ directory, and store that relative path in SQLite.
...
C. Frontend UI (index.html)
    ...
    Dashboard Display: Iterate over the recipes in SQLite to display "Recipe Cards." Each card must show:
